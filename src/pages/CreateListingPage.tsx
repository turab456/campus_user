...
export const CreateListingPage: React.FC = () => {
  ...
  const validateStep = (currentStep: number): boolean => {
    ...
    if (currentStep === 3) {
      if (formData.images.length === 0) errs.images = 'Please select at least one photo';
    }
  ...
  const handleImageToggle = (url: string) => {
    setFormData(prev => {
      ...
      const updated = exists
        ? prev.images.filter(x => x !== url)
        : [...prev.images, url].slice(0, 3); // limit to 3 photos
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const listingData = {
        ...
        description: formData.description || `Listing for ${formData.title} in category ${formData.category}.`,
      ...
      {step === 3 && (
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
            ...
            {/* Mock custom photo upload input */}
            <div className="aspect-square rounded-xl border-2 border-dashed border-slate-300 hover:border-primary transition-colors flex flex-col items-center justify-center text-center p-2 cursor-pointer bg-slate-50">
